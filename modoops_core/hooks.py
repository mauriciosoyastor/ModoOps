...
def _ensure_pos_payment_methods(env):
    ...
    if not configs:
        configs = Config.create({"name": "Mostrador modoops"})
    ...
    if module:
        ...
        if action:
            ...
            if existing:
                ...
                Tile.create(values)
    env["mo.app.tile"].setup_launcher_tile_accents()
    env["mo.hub.card"].setup_sales_hub_card_accents()
    env["mo.hub.card"].setup_inventory_hub_card_accents()
    env["mo.hub.card"].setup_purchase_hub_card_accents()
    env["mo.hub.card"].setup_accounting_hub_card_accents()
