...
from .mo_workshop_logic import normalize_serial


class SgAppliance(models.Model):
    _name = "mo.appliance"
    _description = "Artefacto de taller modoops"
    ...
    work_order_ids = fields.One2many(
        "mo.work.order", "appliance_id", string="Órdenes de trabajo"
    )
    work_order_count = fields.Integer(compute="_compute_work_order_count")
    display_name = fields.Char(compute="_compute_display_name", store=True)

    _mo_appliance_serial_uniq = models.Constraint(
