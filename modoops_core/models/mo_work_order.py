...
class SgWorkOrder(models.Model):
    _name = "mo.work.order"
    _description = "Orden de trabajo de taller modoops"
    ...
    appliance_id = fields.Many2one(
        "mo.appliance", required=True, ondelete="restrict", index=True
    )
    ...
    def get_report_brand_mark_src(self):
        """Data-URI del símbolo modoops para el PDF (no depende de HTTP static)."""
        self.ensure_one()
        return self.env["report.modoops.brand"].get_mark_src()
    ...
    def _recompute_amount_collected(self):
        Move = self.env["mo.cash.movement"]
        for wo in self:
            moves = Move.search(
    ...
    def action_collect_cash(self, amount, medium="cash", note=False):
        ...
        Session = self.env["mo.cash.session"]
        ...
        move = self.env["mo.cash.movement"].create(
            {
    ...
    def create_from_shell(self, vals):
        ...
        Appliance = self.env["mo.appliance"]
