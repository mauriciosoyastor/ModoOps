...
class SgHubCard(models.Model):
    ...
    def _eval_domain(self, domain_str):
        ...
        try:
            ...
            _logger.warning("Dominio inválido en mo.hub.card %s: %s", self.id, domain_str)
            return []
    ...
    def _compute_metric_raw(self):
        ...
        try:
            ...
            _logger.exception("Error calculando métrica para mo.hub.card %s", self.id)
            return None
    ...
    def _get_metric_display(self):
        self.ensure_one()
        metrics_cache = self.env.context.get("_mo_hub_metrics_cache")
