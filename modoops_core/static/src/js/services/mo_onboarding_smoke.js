...
import { selectorsForSurface } from "./mo_pos_theme.js";
import {
    advanceTour,
    getCurrentStep,
    listTourSteps,
} from "./mo_onboarding_tour.js";

export const SMOKE_QUERY_PARAM = "mo_onboarding";
export const SMOKE_STORAGE_KEY = "mo_onboarding_smoke";
...
const APP_TARGET_SELECTORS = {
    "rail.sections": [".mo-rail__sections"],
    "rail.back_to_hub": [".mo-rail__back"],
    "rail.apps": [".mo-rail__apps"],
    "rail.toggle": [".mo-rail__toggle"],
    "hub.kpi_card": [".mo-launcher-shell--hub .mo-launcher-tile"],
    "tile.punto_de_venta": [".mo-rail__app", ".mo-launcher-tile"],
...
export function advanceSmokeTour(session, { targetFound = true, log } = {}) {
    ...
    if (!targetFound && typeof log === "function") {
        log(
            `mo_onboarding_smoke: skip missing target ${current.id} (${current.targetHint})`
        );
    }
