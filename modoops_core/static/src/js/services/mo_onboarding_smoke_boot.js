...
} from "./mo_onboarding_smoke";
import {
    createTourSession,
    getCurrentStep,
    isTourComplete,
    skipTour,
} from "./mo_onboarding_tour";
import { reportShellPath, shellPathDomAttrs } from "./mo_shell_path";
...
export function mountOnboardingSmokeOverlay({
    ...
    if (
        !isOnboardingSmokeEnabled({
            ...
            devAssets: Boolean(devAssets || sessionInfo.mo_dev_assets),
        })
    ) {
        return null;
    }
    ...
    function renderPanel(view, { waiting = false } = {}) {
        const badge = view.badge
            ...
            : "";
        const previewLabel = track === "pos" ? "Mostrador" : "modoops";
