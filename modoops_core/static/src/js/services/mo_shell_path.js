...
import { resolveTourTargetSelectors } from "./mo_onboarding_smoke.js";
...
const APP_CHECKPOINTS = [
    {
        ...
        label: "Inicio",
        surface: "backend",
        targetHint: ".mo-launcher-shell--root",
        required: true,
    },
    {
        id: "shell.hub",
        label: "Hub",
        surface: "backend",
        targetHint: ".mo-launcher-shell--hub",
        required: true,
    },
    {
        id: "shell.section",
...
export function reportShellPath(queryAll, { track = "app", log } = {}) {
    ...
    if (typeof log === "function") {
        for (const result of report.results) {
            if (!result.found) {
                log(`mo_shell_path: missing ${result.id}`);
            }
        }
        log(
            `mo_shell_path: ${report.ok ? "ok" : "incomplete"} (${track})`
        );
    }
    return report;
}
