...
import { createHostController } from "./mo_onboarding_host";
import { mountOnboardingHostOverlay } from "./mo_onboarding_host_boot";
import {
    isDesktopViewport,
    shouldAutoStartQuick,
} from "./mo_onboarding_persist";
import { isOnboardingSmokeEnabled } from "./mo_onboarding_smoke";
...
registry.category("services").add("mo_onboarding", sgOnboardingHostService);
...
function menuItem(id, description, sequence, callbackName) {
    return (env) => ({
        ...
        callback: () => {
            const svc = env.services.mo_onboarding;
            svc?.[callbackName]?.();
        },
        sequence,
    });
...
registry
    ...
    .add(
        "mo_recorrido_rapido",
        menuItem("mo_recorrido_rapido", "Recorrido rápido", 55, "replayQuick")
    );
registry
    .category("user_menuitems")
    .add(
        "mo_recorrido_completo",
        menuItem("mo_recorrido_completo", "Recorrido completo", 56, "replayFull")
    );
