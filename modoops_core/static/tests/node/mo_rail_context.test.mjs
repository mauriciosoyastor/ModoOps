...
import {
    ...
    HOME_ACTIVE_TAG,
} from "../../src/js/chrome/mo_rail_context.js";

const RAIL_NAV_XML = join(
    dirname(fileURLToPath(import.meta.url)),
    "../../src/js/chrome/mo_rail_nav.xml"
);

const HUB_TAGS = {
    modoops_sales_hub: "sales",
    modoops_inventory_hub: "inventory",
};
...
describe("rail context after action settles", () => {
    it("marks hub app active with sections once controller has settled", async () => {
        let actionTag = "modoops_app_launcher";
        let ctx;
        ...
        assert.equal(ctx.activeTag, "modoops_sales_hub");
        assert.equal(
            isRailTileActive(ctx.activeTag, { client_tag: "modoops_sales_hub" }),
            true
        );
    });
});
...
describe("rail on launcher", () => {
    it("treats launcher as home without hub sections or active tiles", () => {
        const ctx = resolveRailNavContext({
            actionTag: "modoops_app_launcher",
            ...hubDeps(),
        });
...
describe("rail after KPI card opens native action", () => {
    it("keeps inventory hub return context when act_window has no tag", () => {
        ...
        assert.equal(
            isRailTileActive(ctx.activeTag, { client_tag: "modoops_inventory_hub" }, returnCtx.hubTag),
            true
        );
        assert.equal(
            isRailTileActive(ctx.activeTag, { client_tag: "modoops_sales_hub" }, returnCtx.hubTag),
            false
        );
    });
...
describe("rail sync after shell leaves (KPI → native)", () => {
    it("shows Volver al hub once controller leaves the hub tag", async () => {
        let actionTag = "modoops_inventory_hub";
        const returnCtx = {
            app: "inventory",
            section: "summary",
            hubTag: "modoops_inventory_hub",
    ...
    it("recovers Volver al hub when shell change fires after probes are exhausted", () => {
        ...
        actionTag = "";
        bus.trigger("mo_RAIL:SHELL_CHANGE");
