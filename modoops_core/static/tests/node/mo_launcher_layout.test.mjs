...
} from "../../src/js/launcher/mo_launcher_layout.js";

const LAUNCHER_SCSS = join(
    dirname(fileURLToPath(import.meta.url)),
    "../../src/scss/modoops_launcher.scss"
);

function assertBodyPaddingTop(scss, shellModifier, expected) {
    const re = new RegExp(
        `\\.mo-launcher-shell--${shellModifier}\\s*\\{[\\s\\S]*?\\.mo-launcher-body\\s*\\{[\\s\\S]*?padding-top:\\s*([^;]+);`
    );
    const match = scss.match(re);
    assert.ok(match, `${shellModifier} .mo-launcher-body padding-top rule missing`);
