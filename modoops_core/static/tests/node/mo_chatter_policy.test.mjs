...
import {
    ...
    noChatterXmlIds,
    phaseForXmlId,
} from "../../src/js/services/mo_chatter_policy.js";

const BACKEND_SCSS = join(
    dirname(fileURLToPath(import.meta.url)),
    "../../src/scss/modoops_backend.scss"
);
...
describe("chatter policy layout anchors (ADR 0015 / S2)", () => {
    it("scopes full-width sheet and residual chatter hide under .mo-form-no-chatter", () => {
        const scss = readFileSync(BACKEND_SCSS, "utf8");
        assert.ok(
            scss.includes(".mo-form-no-chatter"),
            "canonical class .mo-form-no-chatter must exist"
        );
        assert.ok(
