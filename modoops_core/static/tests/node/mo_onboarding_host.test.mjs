...
} from "../../src/js/services/mo_onboarding_host.js";
...
describe("onboarding host controller (ADR 0009/0010)", () => {
    ...
    it("skipActive marks persist flags for the playlist", () => {
        ...
        host.skipActive();
        assert.equal(host.getActivePlaylist(), null);
        assert.equal(storage.getItem("mo_onboarding_full_done"), "1");
        assert.equal(storage.getItem("mo_onboarding_app_done"), "1");
        assert.equal(storage.getItem("mo_onboarding_pos_done"), null);
    });

    it("replay clears flag then starts playlist", () => {
        const storage = memoryStorage();
        storage.setItem("mo_onboarding_app_done", "1");
        const host = createHostController({ storage });
        const result = host.replay("quick");
        assert.equal(result.ok, true);
        assert.equal(storage.getItem("mo_onboarding_app_done"), null);
        assert.equal(host.getActivePlaylist(), "quick");
    });
