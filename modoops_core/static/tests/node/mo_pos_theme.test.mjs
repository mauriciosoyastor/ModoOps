...
import {
    ...
    selectorsForSurface,
    tokenRoleForSurface,
} from "../../src/js/services/mo_pos_theme.js";

const POS_SCSS = join(
    dirname(fileURLToPath(import.meta.url)),
    "../../src/scss/modoops_pos.scss"
);
...
describe("POS Liquid Glass theme contract (ADR 0005)", () => {
    ...
    it("keeps selected order-line visible on the dark ticket pane", () => {
        // modoops densify used background:transparent on all .orderline and
        ...
        assert.ok(
            scss.includes("--mo-pos-orderline-selected") ||
                scss.includes("sg-pos-orderline-selected"),
            "selected line needs a named modoops highlight token"
        );
    ...
    it("exposes order-level Desc. control as discount surface (ADR 0014)", () => {
        ...
        assert.ok(
            ...
            "modoops_pos.scss must style Desc. button"
        );
    });
    ...
    it("keeps SCSS overrides aligned with bg utilities in the contract", () => {
        ...
        for (const name of bgUtilitiesToOverride()) {
            assert.ok(
                scss.includes(`.${name}`),
                `modoops_pos.scss must target .${name} (ADR 0005)`
            );
        }
        for (const id of ["chrome-navbar", "chrome-canvas", "ticket-pane", "product-pane"]) {
            for (const selector of selectorsForSurface(id)) {
                ...
                assert.ok(
                    ...
                    `modoops_pos.scss missing rule for ${selector}`
                );
            }
        }
    });
...
describe("POS UX phases — product card (ADR 0012 Fase A)", () => {
    ...
    it("anchors product-card typography in modoops_pos.scss", () => {
        const scss = readFileSync(POS_SCSS, "utf8");
        assert.ok(
    ...
    it("anchors product image fallback styling in modoops_pos.scss", () => {
        const scss = readFileSync(POS_SCSS, "utf8");
        assert.ok(
        ...
        assert.ok(
            ...
            "missing-image tiles need modoops fallback (Odoo uses .no-image)"
        );
        assert.ok(
            scss.includes("sg-pos-product-fallback") ||
                scss.includes("--mo-pos-product-fallback"),
            "fallback uses a named modoops token/marker for the empty tile"
        );
    });
});
...
describe("POS UX phases — Acciones modal (ADR 0012 Fase B)", () => {
    ...
    it("anchors Acciones hierarchy in modoops_pos.scss", () => {
        const scss = readFileSync(POS_SCSS, "utf8");
        assert.ok(
        ...
        assert.ok(
            scss.includes("--mo-pos-acciones-destructive") ||
                scss.includes("sg-pos-acciones-destructive"),
            "destructive action needs a named modoops token/marker"
        );
        assert.ok(
...
describe("POS UX phases — ticket empty + order lines (ADR 0012 Fase C)", () => {
    ...
    it("anchors ticket empty-state and denser order lines in modoops_pos.scss", () => {
        const scss = readFileSync(POS_SCSS, "utf8");
        assert.ok(
        ...
        assert.ok(
            scss.includes("--mo-pos-ticket-empty") ||
                scss.includes("sg-pos-ticket-empty"),
            "empty ticket needs a named modoops token/marker"
        );
        assert.ok(
...
describe("POS payment theme — chrome + due (ADR 0013 Fase A)", () => {
    ...
    it("anchors payment chrome and due contrast in modoops_pos.scss", () => {
        const scss = readFileSync(POS_SCSS, "utf8");
        assert.ok(
        ...
        assert.ok(
            scss.includes("--mo-pos-payment-due") || scss.includes("sg-pos-payment-due"),
            "Due amount needs a named modoops token/marker"
        );
        assert.ok(
            scss.includes("text-dark") || scss.includes("--mo-text-on-dark"),
...
describe("POS payment theme — methods + validate (ADR 0013 Fase B)", () => {
    ...
    it("anchors payment-line dark chip contrast in modoops_pos.scss", () => {
        const scss = readFileSync(POS_SCSS, "utf8");
        assert.ok(
        ...
        assert.ok(
            scss.includes("--mo-pos-payment-line") ||
                scss.includes("sg-pos-payment-line"),
            "payment line chip needs a named modoops bg token/marker"
        );
    });
    ...
    it("anchors methods, lines, and Validate vs Back in modoops_pos.scss", () => {
        const scss = readFileSync(POS_SCSS, "utf8");
        assert.ok(
        ...
        assert.ok(
            scss.includes("--mo-pos-payment-validate") ||
                scss.includes("sg-flame-cta") ||
                scss.includes("sg-flame-gradient"),
            "Validate must use modoops flame / primary language"
        );
        assert.ok(
...
describe("POS receipt theme — chrome + done (ADR 0013 Fase C)", () => {
    ...
    it("anchors receipt chrome, paper pane, and Done CTA in modoops_pos.scss", () => {
        const scss = readFileSync(POS_SCSS, "utf8");
        assert.ok(
        ...
        assert.ok(
            scss.includes("--mo-pos-receipt-paper") ||
                scss.includes("sg-pos-receipt-paper"),
            "printed ticket stays light paper on dark canvas"
        );
        assert.ok(
            scss.includes("--mo-pos-receipt-done") ||
                scss.includes("sg-flame-cta") ||
                scss.includes("sg-flame-gradient"),
            "New Order / Done must use modoops flame CTA"
        );
    });
});
