...
} from "../../src/js/services/mo_onboarding_smoke.js";
import { reportShellPath, shellPathDomAttrs } from "../../src/js/services/mo_shell_path.js";
import { selectorsForSurface } from "../../src/js/services/mo_pos_theme.js";
import {
    createTourSession,
    getCurrentStep,
    isTourComplete,
} from "../../src/js/services/mo_onboarding_tour.js";

describe("onboarding smoke", () => {
    it("enables smoke when URL has mo_onboarding=1", () => {
        assert.equal(
            isOnboardingSmokeEnabled({
                search: "?mo_onboarding=1",
    ...
    it("enables smoke when localStorage flag is set", () => {
        assert.equal(
            isOnboardingSmokeEnabled({
                search: "",
                storage: { getItem: (k) => (k === "mo_onboarding_smoke" ? "1" : null) },
            }),
            true
        );
    });
    ...
    it("resolves app targetHints to DOM selectors", () => {
        assert.deepEqual(resolveTourTargetSelectors(".mo-launcher-shell--root"), [
            ".mo-launcher-shell--root",
        ]);
    ...
    it("hub.kpi_card resolves only to KPI tiles inside the hub shell", () => {
        assert.deepEqual(resolveTourTargetSelectors("hub.kpi_card"), [
            ".mo-launcher-shell--hub .mo-launcher-tile",
        ]);
        const hubTile = { textContent: "Pedidos", id: "hub-kpi" };
        const homeTile = { textContent: "Ventas", id: "home-tile" };
        const queryAll = (sel) => {
            if (sel === ".mo-launcher-shell--hub .mo-launcher-tile") {
                return [hubTile];
            }
            if (sel === ".mo-launcher-tile") {
    ...
    it("appearing hub KPI tiles does not complete the app.kpi step", () => {
        ...
        const queryAll = (sel) =>
            sel === ".mo-launcher-shell--hub .mo-launcher-tile" ? [hubTile] : [];
