...
} from "../../src/js/services/mo_shell_path.js";
import { resolveTourTargetSelectors } from "../../src/js/services/mo_onboarding_smoke.js";
...
describe("shell path (ADR 0007)", () => {
    ...
    it("evaluateCheckpoint reports found when target is in DOM", () => {
        ...
        const queryAll = (sel) =>
            sel === ".mo-launcher-shell--root" ? [{ tag: "div" }] : [];
        const result = evaluateCheckpoint(queryAll, home);
        assert.equal(result.id, "shell.home");
        assert.equal(result.found, true);
        assert.equal(result.selectorUsed, ".mo-launcher-shell--root");
    });
    ...
    it("shell.kpi uses hub-scoped KPI card hint (ADR 0008)", () => {
        ...
        const queryAll = (sel) =>
            sel === ".mo-launcher-shell--hub .mo-launcher-tile"
                ? [{ textContent: "Pedidos" }]
                : [];
