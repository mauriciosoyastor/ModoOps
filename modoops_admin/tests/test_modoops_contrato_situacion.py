...
class SituacionTests(unittest.TestCase):
    ...
    def test_listo_cobrar_saldo(self):
        sem, _ = mod.situacion_contrato(
            contrato_state="vigente", hito1_ok=True, hito1_cobrado=True,
            hito2_ok=True, saldo_cobrado=False, today=date(2026, 9, 3)
        )
    ...
    def test_hipercare(self):
        sem, _ = mod.situacion_contrato(
            contrato_state="vigente", hito1_ok=True, hito1_cobrado=True,
            hito2_ok=True, saldo_cobrado=True,
    ...
    def test_hito1_pendiente_detalle_25(self):
        sem, det = mod.situacion_contrato(
            contrato_state="vigente", hito1_ok=False, today=date(2026, 9, 3)
        )
        self.assertEqual(sem, "en_implementacion")
        self.assertIn("Hito 1", det)
        self.assertIn("25%", det)

    def test_listo_cobrar_hito1(self):
        sem, det = mod.situacion_contrato(
            contrato_state="vigente", hito1_ok=True, hito1_cobrado=False,
            today=date(2026, 9, 3),
        )
        self.assertEqual(sem, "listo_cobrar_hito1")
        self.assertIn("25%", det)

    def test_en_go_live_tras_hito1_cobrado(self):
        sem, det = mod.situacion_contrato(
            contrato_state="vigente", hito1_ok=True, hito1_cobrado=True,
            hito2_ok=False, today=date(2026, 9, 3),
        )
        self.assertEqual(sem, "en_go_live")
        self.assertIn("25%", det)

    def test_listo_cobrar_saldo_25_final(self):
        sem, det = mod.situacion_contrato(
            contrato_state="vigente", hito1_ok=True, hito1_cobrado=True,
            hito2_ok=True, saldo_cobrado=False, today=date(2026, 9, 3)
        )
    ...
    def test_suspendido_rehab_en_jornada(self):
        sem, det = mod.situacion_contrato(
        ...
        self.assertIn("dentro de la jornada", det)

    def test_suspendido_espeja_tenant(self):
        sem, _ = mod.situacion_contrato(
            tenant_state="suspendido", contrato_state="vigente",
            hito1_ok=True, hito1_cobrado=True,
            hito2_ok=True, saldo_cobrado=True, today=date(2026, 9, 3))
        self.assertEqual(sem, "suspendido")
