...
3. ModoOps + vas a abrir varios context:
     → skill laya-recortador / recortar_client.py (ensure daemon + ≤2 expand)
     → NO expandas todos los processes del query (tapa symbol_count miente: #209)
4. context({name, file_path}) SOLO de expand[]     → Deep dive
5. READ gitnexus://repo/{name}/process/{name}      → Trace full execution flow (si hace falta)
```

> If step 2 says "Index is stale" → run `node .gitnexus/run.cjs analyze` in terminal.  
> **ModoOps:** si el query trae ≥3 processes, **obligatorio** pasar por Laya (`tools/laya/recortar_client.py`) antes de `context`. El dedupe de `process_symbols` deja attach vacío en hubs; Laya recovery/fill (#206) lo mitiga en sesión — no reindexees por eso ([#213](https://github.com/mauriciosoyastor/ModoOps/issues/213)).
...
- [ ] query **o** recortar_client (ModoOps: cliente si ≥3 processes)
- [ ] context solo de expand / ≤2 símbolos clave
- [ ] READ process resource for full execution traces (opcional)
