...
**When to run:** First time in a project, after major code changes, or when `gitnexus://repo/{name}/context` reports the index is stale. In Claude Code / Cursor, a PostToolUse hook detects staleness after `git commit|merge|rebase|cherry-pick|pull` and **notifies** the agent to run `analyze` — the hook does **not** run analyze itself (timeout / Ladybug WAL risk). ModoOps: `.cursor/hooks.json` → `node tools/gitnexus/hook_notify_stale.mjs`.

**ModoOps contract (mapa #207 / #212):**
- **Notify** after git mutations (hook) and when MCP/status says stale before `query`/Recortador.
- **Incremental** `analyze` in the terminal when you see stale; `--force --embeddings` only if `doctor` parity fails or a human/ensure offline asks.
- **Ensure / smoke (optional):** `node tools/gitnexus/ensure_smoke.mjs` — freshness + doctor + attach-rate warn on 3 fixed queries. Never spawn `analyze` from a hook. Attach &lt;80% with a **fresh** index is `dedupe/structural` (issue #213 / #209), not “reindex again” — use Laya fill (#206).
...
### ensure / smoke (ModoOps)

```bash
node tools/gitnexus/ensure_smoke.mjs
node tools/gitnexus/ensure_smoke.mjs --mode notify
node tools/gitnexus/ensure_smoke.mjs --mode ensure --analyze   # incremental, outside hooks
node tools/gitnexus/ensure_smoke.mjs --mode smoke
```

Evidence: `tools/gitnexus/ensure_smoke_last.json`. Doc: `tools/gitnexus/PROTOTYPE_ENSURE_211.md`.
